...
export class typeChecking implements AseRobotVisitor {
  ...
  visitFunc(node: Func) {
    for (let statement of node.statement) {
      ...
      if (isReturn || (isControlStructure && result != null)) {
        if (
          result.type !=
          this.normalizeType(this.funcs.get(node.name)?.[0].$type)
        ) {
  ...
  visitFunCall(node: FunCall) {
    ...
    const localScope = new Map<string, any>(
      paramValues.map((value, i) => [
        this.program.Func.find((f) => f.name === node.callName)?.parameter[i]
          .name!,
        value,
      ]),
    );
    ...
    if (
      this.normalizeType(returnValue) !== this.normalizeType(returnType.$type)
    ) {
  ...
  visitAssignVar(node: AssignVar) {
    ...
    if (this.normalizeType(value.type) !== this.normalizeType(varType)) {
      throw new Error(
        `Type mismatch in assignment to ${node.var_to_assign.name}`,
      );
    }
  ...
  visitdeclaVar(node: declaVar) {
    if (this.vars.length === 0)
      throw new Error("Variable declaration outside any scope");
